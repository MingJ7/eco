...
// }
export function MongoDBAdapterWithRole(
  /**
   * The MongoDB client.
   *
  ...
  const getDb = async () => {
    ...
    return {
      U: _db.collection<UserWithRole>(c.Users),
  ...
  return {
    async createUser(data) {
      const user = to<UserWithRole>(data)
      user.role = "User"
      await using db = await getDb()
      await db.U.insertOne(user)
      return from<UserWithRole>(user)
    },
    async getUser(id) {
      await using db = await getDb()
    ...
    async updateUser(data) {
      const { _id, ...user } = to<UserWithRole>(data)
      await using db = await getDb()
      ...
      return from<UserWithRole>(result!)
    },
    async deleteUser(id) {
      const userId = _id(id)
      await using db = await getDb()
