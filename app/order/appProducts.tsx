...
export default function AppProducts(param: {setOpen: Function}) {
    ...
    return (
      <div className="dark:text-black content-center">
        {cart.products.map((order, index) => {
          return (
          <div key={index} className={`${index !==0 ? "border-t-0" : "rounded-t-lg"} ${index == cart.products.length-1 ? "rounded-b-lg" : ""} border-4`}>
            ...
            <button 
            className={`${index !==0 ? "rounded-t-sm" : "rounded-sm"} w-full bg-purple-300 px-2 font-semibold`}
            onClick={() => {setItemNumber({type: 'SET', payload: index}); param.setOpen(false)}}>
              Edit
            </button>
            {index !== 0 ? <div><button className="w-full bg-red-300 px-2 rounded-b-sm font-semibold"
            onClick={() => {
              if (itemNumber === index) setItemNumber({type: 'SET', payload: 0})
              cartDispatch({type: "REMOVE ORDER", payload: {orderid: index, item:{id:"", name:"", price:0}}
              })}}>Delete</button></div> : null}
          </div>)
        })}
        <p>Total: {cart.products.reduce((acc, cur) => {return {main:null, sides:[], cost: acc.cost+cur.cost}}).cost} SGD</p>
